...
export class UserService {
  ...
  async login(user: UserLoginDto) {
    const userRes = await this.entityManager.findOne(User, {
      where: {
    ...
    if (!userRes) throw new HttpException('用户名不存在', HttpStatus.ACCEPTED);
  ...
  /**
   *
   * @param roleIds 根据角色id 联表查询permission的list
   * @returns
   */
